...
void fuzzAtan2AgainstAtanF(float y, float x) { EXPECT_NEAR(safeAtan2f(y, x), atanf(y / x), kFloatTol); }
...
void fuzzAtan2FiniteF(float y, float x) {
    EXPECT_TRUE(std::isfinite(safeAtan2f(y, x)));
}
FUZZ_TEST(SafeMathFloatFuzz, fuzzAtan2FiniteF)
...
void fuzzAtan2AgainstAtanD(double y, double x) { EXPECT_NEAR(safeAtan2(y, x), atan(y / x), kDoubleTol); }
...
void fuzzAtan2FiniteD(double y, double x) {
    EXPECT_TRUE(std::isfinite(safeAtan2(y, x)));
}
FUZZ_TEST(SafeMathDoubleFuzz, fuzzAtan2FiniteD)
    .WithDomains(fuzztest::Finite<double>(), fuzztest::Finite<double>());
