...
TEST_F(FuzzTest, StateConversions_CartesianRoundTrip) {
    ...
    for (int i = 0; i < numTests; ++i) {
        ...
        Eigen::Vector3d r_original(
            r_mag * std::sin(phi) * std::cos(theta),
            r_mag * std::sin(phi) * std::sin(theta),
            r_mag * std::cos(phi)
        );
...
TEST_F(FuzzTest, PhysicalConstraints_OrbitalElements) {
    ...
    for (int i = 0; i < numTests; ++i) {
        ...
        // h = sqrt(mu * a * (1 - e^2))
        double h_expected = std::sqrt(muEarth * elements.semiMajorAxis * (1.0 - elements.eccentricity * elements.eccentricity));
