...
// ============================================================================
// Targeted: a time update advances the mean by the MRP kinematics and grows the
// covariance (which stays symmetric + PSD + finite).
// ============================================================================
...
FUZZ_TEST(InertialFilterFuzz, fuzzRegularizationKeepsMrpBounded)
    .WithDomains(fuzztest::InRange(1e-2, 1.0),   // alpha
                 fuzztest::InRange(0.0, 2.0),    // beta
                 fuzztest::InRange(1e-4, 1e-1),  // initial attitude variance
                 fuzztest::InRange(1e-6, 1e-2),  // initial rate variance
