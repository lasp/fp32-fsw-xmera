...
algorithm (``InertialFilterAlgorithm``). The algorithm owns a ``filtering::SRuKF`` from
:ref:`filteringCore`. Configuration is validated through an immutable ``InertialFilterConfig`` and applied
...
``InertialFilterConfig``, owns the ``measurement_queue``, and exposes ``update()``, ``reInitialize()``,
and ``reInitializeAll()``. ``InertialFilter`` is the xmera adapter: it owns the message ports, converts
