...
- Constructing the algorithm in `reset()`
...
The algorithm performs the core mathematical computations and it is designed to be separated from the FSW messaging framework. It operates using the input and output float structs (`AttNavInput`, `AttRefInput`, and `AttGuidOutput`) defined in the algorithm header `attTrackingErrorAlgorithm.h`. The algorithm has no tunable parameters and no internal state.
