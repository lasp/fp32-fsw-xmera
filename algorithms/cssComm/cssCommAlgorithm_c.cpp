...
namespace {
CssCommConfig configFromC(double maxSensorValues[MAX_NUM_CSS_SENSORS], double chebyPolynomials[MAX_NUM_CHEBY_POLYS]) {
    ...
    return CssCommConfig::create(maxValues, cheby);
}
}  // namespace
...
bool CssCommAlgorithm_validateConfig(double maxSensorValues[MAX_NUM_CSS_SENSORS],
    ...
    try {
        (void)configFromC(maxSensorValues, chebyPolynomials);
        return true;
...
CssCommAlgorithmHandle* CssCommAlgorithm_create(double maxSensorValues[MAX_NUM_CSS_SENSORS],
    ...
    return fsw::createHandle<::CssCommAlgorithm, CssCommAlgorithmHandle>(
        configFromC(maxSensorValues, chebyPolynomials));
}
...
void CssCommAlgorithm_setConfig(CssCommAlgorithmHandle* self,
                                ...
                                double chebyPolynomials[MAX_NUM_CHEBY_POLYS]) {
    fsw::fromHandle<::CssCommAlgorithm>(self)->setConfig(configFromC(maxSensorValues, chebyPolynomials));
}
