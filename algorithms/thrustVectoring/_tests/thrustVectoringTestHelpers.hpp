...
inline ThrustVectoringConfig makeConfig(const Eigen::Vector3f& sigma_MB,
                                        ...
                                        float thrust,
                                        float thetaMax = kWideCone) {
    return ThrustVectoringConfig::create(makePlatformConfig(sigma_MB, r_MB_B, thetaMax), thrust, r_CB_B);
}
...
// alignment case: the thrust then fires straight from the joint towards the center of mass.
...
inline void propertyOutputsFinite(const Eigen::Vector3f& sigma_MB,
    ...
    // center of mass too close to the joint for a pointing to exist, or a mounting whose un-deflected thrust fires
    // outboard and would put the thruster inside the vehicle.
    if (!ThrustVectoringConfig::isValidThrust(thrust) || !ThrustVectoringConfig::isValidR_CM(r_CB_B, r_MB_B) ||
        ...
        EXPECT_THROW((void)makeConfig(sigma_MB, r_MB_B, r_CB_B, thrust), fsw::invalid_argument);
        return;
    }
