...
namespace {
/*! Solve for the thrust direction that produces the requested torque Lreq_B about the center of mass, for a thrust
 of the given magnitude acting through the point M that r_MC_B is measured from.
 @return [-] thrust unit direction, body frame
*/
Eigen::Vector3f solveThrustDirection(const Eigen::Vector3f& r_MC_B, float thrust, const Eigen::Vector3f& Lreq_B) {
    const float b = r_MC_B.stableNorm();  // moment arm about M; the configuration guarantees b > kMinR_CM
    const Eigen::Vector3f rHat_MC_B = r_MC_B / b;
    ...
    const Eigen::Vector3f tHat_B = (tPerp_B + tAlong_B).stableNormalized();
