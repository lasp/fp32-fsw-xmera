 ...
 *
 * Bundles the platform mounting geometry, the thrust magnitude and the center-of-mass position, all of which are
 * fixed while the module runs. An instance can only exist if the mounting vectors are finite, the deflection cone
 * half-angle lies in (0, pi), the thrust magnitude is finite and positive, and the center of mass is far enough
 * from the platform joint for the pointing to be defined. Construct via ThrustVectoringConfig::create(...).
 */
class ThrustVectoringConfig final {
   public:
    static ThrustVectoringConfig create(const ThrustVectoringPlatformConfiguration& platformConfig,
        ...
        return {boundedPlatformConfig, thrust, r_CB_B};
    }
    ...
    /*! The thruster is mounted behind the joint along the thrust, so the un-deflected thrust must point from the
     * joint back towards the center of mass for the thruster itself to sit outboard of the joint rather than
     * inside the vehicle. A deflection stays on the correct side of the joint as long as the cone does not reach
     * a right angle. Call only for a geometry isValidR_CM has already accepted, so r_MC has a direction. */
   ...
   private:
    ...
    ThrustVectoringConfig(const ThrustVectoringPlatformConfiguration& platformConfig,
                          float thrust,
                          const Eigen::Vector3f& r_CB_B)
        : platformConfig(platformConfig), thrust(thrust), r_CB_B(r_CB_B) {}
