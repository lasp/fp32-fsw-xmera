...
inline Eigen::Vector3f referenceTorque(const MomentumManagementRwArrayConfiguration& rwArrayConfig,
    ...
    const float hsNorm = hsDumpable_B.norm();

    const Eigen::Vector3f hsToDump_B = (hsNorm >= params.hsMin) ? hsDumpable_B : Eigen::Vector3f::Zero();
...
inline void regressionFuzzMomentumManagement(const Eigen::Vector3f& axis0,
    ...
    const float hsNorm = (params.dumpableProjection_B * clusterMomentum(rwArrayConfig, wheelSpeeds)).norm();
