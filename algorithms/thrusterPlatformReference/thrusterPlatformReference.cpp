...
void ThrusterPlatformReference::reset(uint64_t callTime) {
    ...
    v3SetZero(this->hsInt_M);
    v3SetZero(this->priorHs_M);
...
void ThrusterPlatformReference::updateState(uint64_t callTime) {
    ...
    if (this->momentumDumping == Yes) {
        RWSpeedMsgPayload rwSpeedMsgIn = this->rwSpeedsInMsg();
        ...
        double H[3];
        v3Scale(this->K, hs_M, H);
        if (this->Ki > 0) {
            double Hint[3];
            v3Scale(this->Ki, this->hsInt_M, Hint);
            v3Add(H, Hint, H);
        }
        double d_M[3];
        v3Cross(T_M, H, d_M);
