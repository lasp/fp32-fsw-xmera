...
``reInitialize()`` (which clears both the persistence counter and the latched fault), or trigger a module ``reset()``
(which reconstructs the algorithm and likewise clears all state).

The ``reInitializeExceptPersistentStates()`` method sets the persistence counter back to zero but preserves the latched fault state,
allowing the counter to be cleared independently of the rate source selection. ``reInitialize()`` clears both, and
...
- A declared fault persists across ``reInitializeExceptPersistentStates()`` (which clears only the persistence counter); ``reInitialize()``
