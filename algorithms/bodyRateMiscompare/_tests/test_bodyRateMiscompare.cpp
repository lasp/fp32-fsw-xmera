...
// reInitializeExceptPersistentStates() clears the persistence counter, preventing fault from triggering.
TEST(BodyRateMiscompareTest, ReInitializeClearsPersistenceCounter) {
    ...
    // reInitializeExceptPersistentStates clears the counter
    alg.reInitializeExceptPersistentStates();
...
// reInitializeExceptPersistentStates() does not clear the latched fault state — a detected fault persists.
TEST(BodyRateMiscompareTest, ReInitializePreservesLatchedFault) {
    ...
    // reInitializeExceptPersistentStates only clears the counter, not the latched fault
    alg.reInitializeExceptPersistentStates();
...
// reInitialize re-arms both the persistence counter and the latched fault (unlike reInitializeExceptPersistentStates).
TEST(BodyRateMiscompareTest, ReInitializeAllReArmsLatchedFault) {
    ...
    // reInitialize clears the latch; agreeing inputs no longer report a fault.
    alg.reInitialize();
    auto out = alg.update(imu, imu);
    EXPECT_FALSE(out.bodyRateFaultDetected);
    EXPECT_EQ(out.omega_BN_B, imu);
