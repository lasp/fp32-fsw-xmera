...
class BodyRateMiscompareAlgorithm final {
   public:
    ...
    //! Reset the transient runtime only (the persistence counter). This module's persistent state --
    //! the latched fault (useImuRatesInternal) -- is preserved.
    void reInitializeExceptPersistentStates();
    //! Full reset: reInitializeExceptPersistentStates() plus re-seeding the persistent state (the
    //! latched fault useImuRatesInternal) from the configured useImuRates.
    void reInitialize();
