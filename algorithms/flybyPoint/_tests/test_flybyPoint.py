...
@pytest.mark.parametrize("max_rate", [1.0, 0.01])
@pytest.mark.parametrize("max_acceleration", [0.1, 1E-7])
@pytest.mark.parametrize("pos_knowledge", [0.1, 1E5])
...
def test_flybyPoint_diagnostic_collinearity(
    ...
    flyby_guidance.maxRateThreshold = max_rate
    flyby_guidance.maxAccelerationThreshold = max_acceleration
...
def test_flybyPoint_maxacc(
    ...
    for i in range(round(600 / sim_dt)):
        if(i in trigger_indices):
            pos_vec = np.array(initial_position) + np.array(initial_velocity) * (i * sim_dt) * fac
            position_data.append(pos_vec)
            vel_vec = np.array(initial_velocity) * fac  # large velocity drives predicted acceleration above threshold
            velocity_data.append(vel_vec)
        else:
            position_data.append(np.array(initial_position) + np.array(initial_velocity) * (i * sim_dt))
            velocity_data.append(np.array(initial_velocity))
...
def flybyPointTestFunction(show_plots, initial_position, initial_velocity, filter_dt, orbit_normal_sign,
    ...
    for i in range(len(reference_attitude)):
        ...
        tolerance = 1E-5
        np.testing.assert_allclose(ur_output[i], ur, rtol=0, atol=tolerance, verbose=True)
        np.testing.assert_allclose(ut_output[i], ut, rtol=0, atol=tolerance, verbose=True)
        np.testing.assert_allclose(uh_output[i], uh, rtol=0, atol=tolerance, verbose=True)
        np.testing.assert_allclose(reference_rate[i], omega, rtol=0, atol=tolerance, verbose=True)
        np.testing.assert_allclose(reference_acceleration[i], omegaDot, rtol=0, atol=tolerance, verbose=True)
