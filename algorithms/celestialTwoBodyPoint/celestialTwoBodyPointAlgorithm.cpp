...
AttRefMsgF32Payload CelestialTwoBodyPointAlgorithm::update(EphemerisMsgF32Payload& celBodyIn,
    ...
    if (this->secCelBodyIsLinked) {
        ...
        float dotProduct = R_P2B_N.normalized().dot(R_P1B_N.normalized());
    ...
    Eigen::Vector3f r1_N_hat = R_P1B_N.normalized().cast<float>();
    Eigen::Vector3f r3_N_hat = R_N.normalized().cast<float>();
    Eigen::Vector3f r2_N_hat = (r3_N_hat.cross(r1_N_hat)).normalized().cast<float>();
    ...
    /* - MRP computation */
    Eigen::Vector3f sigma_RN = dcmToMrp(dcm_RN);
    eigenVectorToCArray(sigma_RN, attRefOut.sigma_RN);

    /* - Reference base-vectors first time-derivative */
    Eigen::Vector3f dr1_N_hat =
        (Eigen::Matrix3f::Identity() - r1_N_hat * r1_N_hat.transpose()) * (v_P1B_N / R_P1B_N.norm()).cast<float>();
    Eigen::Vector3f dr3_N_hat = (Eigen::Matrix3f::Identity() - r3_N_hat * r3_N_hat.transpose()) * (v_N / R_N.norm()).cast<float>();
    Eigen::Vector3f dr2_N_hat = dr3_N_hat.cross(r1_N_hat) + r3_N_hat.cross(dr1_N_hat);
    ...
    Eigen::Vector3f domega_RN_N = dcm_RN.transpose() * domega_RN_R;
