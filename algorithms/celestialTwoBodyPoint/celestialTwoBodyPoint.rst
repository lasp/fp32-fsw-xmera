...
- The **algorithm** (``CelestialTwoBodyPointAlgorithm.h``/``.cpp``) is a pure C++23 class with no framework
  dependencies. Its ``update()`` takes the primary, secondary, and spacecraft inertial states (each an
  ``InertialStateInput`` of position and velocity) and returns a ``CelestialTwoBodyPointOutput`` struct. It must
  not throw from ``update()``.

A pure-C shim (``celestialTwoBodyPointAlgorithm_c.h``/``.cpp``) wraps the algorithm class for use by Ada/Adamant
...
.. admonition:: Edge Case Guard 2
    ...
    .. admonition:: Edge Case Guard 3
        ...
        This edge case falls within Edge Case 2. The Edge Case 2 fallback constraint axis
        :math:`\mathbf{r}_{P/B} \times \dot{\mathbf{r}}_{P/B}` is zero and undefined if the relative position vector
        :math:`\mathbf{r}_{P/B}` and the relative velocity vector :math:`\dot{\mathbf{r}}_{P/B}` are collinear, or if
        the relative velocity :math:`\dot{\mathbf{r}}_{P/B}` is itself (near) zero. Both are guarded: the angle
        between the vectors must exceed a ``kSmallAngle`` threshold, and the squared norm of
        :math:`\dot{\mathbf{r}}_{P/B}` must exceed ``kMinNormSq``. If either check fails, the algorithm returns an
        identity reference attitude and zero reference rates.
...
- Edge Case 3: (within Edge Case 2) The relative position vector :math:`\mathbf{r}_{P/B}` and the relative velocity
  vector :math:`\dot{\mathbf{r}}_{P/B}` must not be collinear, and :math:`\dot{\mathbf{r}}_{P/B}` must not be
  (near) zero, or the Edge Case 2 fallback constraint axis :math:`\mathbf{r}_{P/B} \times \dot{\mathbf{r}}_{P/B}`
  will be zero and undefined. In that case identity reference attitude and zero reference rates are returned.
- Position and velocity inputs are read in double precision to preserve orbit-scale accuracy. The attitude /
  rate output is single-precision; within the trackable-rate regime expect a relative accuracy of
  :math:`\sim 10^{-6}` on :math:`\boldsymbol{\sigma}_{\mathcal{R/N}}`,
  :math:`\boldsymbol{\omega}_{\mathcal{R/N}}`, and :math:`\dot{\boldsymbol{\omega}}_{\mathcal{R/N}}`.
