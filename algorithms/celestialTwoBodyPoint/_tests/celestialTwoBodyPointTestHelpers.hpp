...
inline void testCelestialTwoBodyPointRegression(const Eigen::Vector3d& r_PN_N,
    ...
    const auto tolFor = [&](float absTol, float expectedVal) {
        return std::max(absTol, relTol * std::abs(expectedVal));
    };
    for (int i = 0; i < 3; ++i) {
        EXPECT_NEAR(result.sigma_RN[i], expected.sigma_RN[i], tolFor(sigmaAbsTol, expected.sigma_RN[i]));
        EXPECT_NEAR(result.omega_RN_N[i], expected.omega_RN_N[i], tolFor(rateAbsTol, expected.omega_RN_N[i]));
        EXPECT_NEAR(result.domega_RN_N[i], expected.domega_RN_N[i], tolFor(rateAbsTol, expected.domega_RN_N[i]));
