...
inline void testCelestialTwoBodyPointRegression(const Eigen::Vector3d& r_PN_N,
    ...
    /*! The reference angular rate/acceleration are only physically trackable below a generous
        ceiling; above it the geometry is un-physical and the algorithm's float R->N rotation
        legitimately diverges from the double reference. Skip those cases (finiteness across the
        full domain is covered by propertyOutputIsFinite). */
    constexpr float kTrackableCeiling = 10.0F;  // [rad/s], [rad/s^2] — far above any real spacecraft
    for (int i = 0; i < 3; ++i) {
        if (std::abs(expected.omega_RN_N[i]) > kTrackableCeiling ||
            std::abs(expected.domega_RN_N[i]) > kTrackableCeiling) {
            return;
        }
    }

    /*! Combined absolute + relative tolerance. The absolute floor handles near-zero outputs; the
        relative term scales with magnitude (one float32 ULP can exceed a fixed absolute tol at large
        magnitude). sigma is exact to float precision; omega/domega carry the float R->N rotation
        error (~1e-7 * |output|), so they take a looser absolute floor. */
    constexpr float relTol = 1e-5F;
    constexpr float sigmaAbsTol = 1e-5F;
    constexpr float rateAbsTol = 1e-4F;
    const auto tolFor = [&](float absTol, double expectedVal) {
        return std::max(absTol, relTol * std::abs(static_cast<float>(expectedVal)));
    };
    for (int i = 0; i < 3; ++i) {
        EXPECT_NEAR(result.sigma_RN[i], static_cast<float>(expected.sigma_RN[i]), tolFor(sigmaAbsTol, expected.sigma_RN[i]));
        EXPECT_NEAR(result.omega_RN_N[i], static_cast<float>(expected.omega_RN_N[i]), tolFor(rateAbsTol, expected.omega_RN_N[i]));
        EXPECT_NEAR(result.domega_RN_N[i], static_cast<float>(expected.domega_RN_N[i]), tolFor(rateAbsTol, expected.domega_RN_N[i]));
