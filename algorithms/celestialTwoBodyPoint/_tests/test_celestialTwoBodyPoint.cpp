...
// When the celestial bodies are aligned and the primary relative velocity is zero, the fallback constraint axis
// (r_PB_N x v_PB_N) is undefined; identity reference attitude and zero reference rates are returned instead of NaN.
TEST(CelestialTwoBodyPointTest, CelestialBodiesAlignedZeroRelativeVelocityReturnsZero) {
    const Eigen::Vector3d r_PN_N{1e4, 0.0, 0.0};
    const Eigen::Vector3d v_PN_N{1e4, 0.0, 0.0};
    const Eigen::Vector3d r_SN_N{-1e4, 0.0, 0.0};
    const Eigen::Vector3d v_SN_N{0.0, 3e4, 0.0};
    const Eigen::Vector3d r_BN_N{5e2, 0.0, 0.0};
    const Eigen::Vector3d v_BN_N{1e4, 0.0, 0.0}; /* v_PB_N = v_PN_N - v_BN_N = 0 */
    const float celestialBodyAlignmentThreshold = 0.017F;
