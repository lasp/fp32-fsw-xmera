...
*/
void GimbalAnglesToMotorAngles::reset(uint64_t currentSimNanos) {
    if (!this->twoAxisGimbalInMsg.isLinked()) {
        throw std::invalid_argument("gimbalAnglesToMotorAngles.twoAxisGimbalInMsg wasn't connected.");
    }

    const auto config =
        GimbalAnglesToMotorAnglesConfig::create(StepperMotorAngleRange{this->minAngle, this->maxAngle},
                                                ...
                                                GimbalToMotorAngleTableLayout{this->rowStartStrideIndices,
                                                                              ...
                                                                              this->tableStepAngle});
    this->algorithm = std::make_unique<GimbalAnglesToMotorAnglesAlgorithm>(config);
    this->previousWrittenTime = -1.0;
}

GimbalAnglesToMotorAnglesConfig GimbalAnglesToMotorAngles::toConfig() const {
    return GimbalAnglesToMotorAnglesConfig::create(StepperMotorAngleRange{this->minAngle, this->maxAngle},
                                                   this->gimbalToMotor1AngleData,
                                                   this->gimbalToMotor2AngleData,
                                                   GimbalToMotorAngleTableLayout{this->rowStartStrideIndices,
                                                                                 this->rowStartColIndices,
...
void GimbalAnglesToMotorAngles::reconfigure() const {
    if (!this->algorithm) {
        throw XmeraLifecycleException("GimbalAnglesToMotorAngles reset() has not been called.");
    }

    this->algorithm->setConfig(this->toConfig());
}

void GimbalAnglesToMotorAngles::reInitialize() {
    if (!this->algorithm) {
        throw XmeraLifecycleException("GimbalAnglesToMotorAngles reset() has not been called.");
    }
...
*/
void GimbalAnglesToMotorAngles::updateState(uint64_t currentSimNanos) {
    if (!this->algorithm) {
        throw XmeraLifecycleException("GimbalAnglesToMotorAngles reset() has not been called.");
    }
