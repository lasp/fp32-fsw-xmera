...
inline Eigen::Vector3f referenceTriad(const Eigen::Vector3f& rHat_SB_N,
    ...
    /*! Return identity attitude if solar array drive axis and thrust direction are nearly parallel or if either
     * thrustHat_B or rHat_SB_N are zero */
    ...
    if (fabsf(sunToThrustRefAngle) < kParallelThresholdRad) {
        ...
        /*! No rebuild needed here: n3Hat_N is an exact axis vector, so this cross product does not
         * cancel and leaves no amplified error behind. */
        ...
        /*! Rebuild the first axis from the two clean ones to remove the component the first cross
         * product leaves along the axis it was crossed with. See triad.rst. */
        d1Hat_N = d2Hat_N.cross(d3Hat_N).normalized();
    }
...
inline void testTriadRegression(const Eigen::Vector3f& rHat_SB_N,
                                const Eigen::Vector3f& thrustHat_B,
                                const Eigen::Vector3f& sadaHat_B,
                                const Eigen::Vector3f& thrustReqHat_N,
                                const N3Axis n3Axis) {
...
inline void propertyOutputIsFinite(const Eigen::Vector3f& rHat_SB_N,
                                   const Eigen::Vector3f& thrustHat_B,
                                   const Eigen::Vector3f& sadaHat_B,
                                   const Eigen::Vector3f& thrustReqHat_N,
                                   const N3Axis n3Axis) {
