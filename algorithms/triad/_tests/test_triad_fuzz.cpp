...
void fuzzTriadRegression(const Eigen::Vector3f& rHat_SB_N,
    ...
    if (signOfN3Hat_N == 0.0F || sadaHat_B.stableNorm() == 0.0F || thrustReqHat_N.stableNorm() == 0.0F) {
        return;
    }
...
FUZZ_TEST(TriadAlgorithmFuzz, fuzzTriadRegression)
    .WithDomains(xmera::fuzz::Vector3fInRange(-1e6F, 1e6F),  // rHat_SB_N
                 ...
                 fuzztest::InRange(-1e6F, 1e6F));            // signOfN3Hat_N
...
void fuzzPropertyThrustBodyHeadingAlignedToThrustInertialHeading(const Eigen::Vector3f& rHat_SB_N,
    ...
    if (signOfN3Hat_N == 0.0F || sadaAxisToThrustAngle < kParallelThresholdRad ||
        thrustHatUnit_B.stableNorm() == 0.0F || rHatUnit_SB_N.stableNorm() == 0.0F || sadaHat_B.stableNorm() == 0.0F ||
        thrustReqHat_N.stableNorm() == 0.0F) {
        return;
    }
    ...
    const Eigen::Vector3f n3Hat_N = (copysignf(1.0F, signOfN3Hat_N) * Eigen::Vector3f::UnitZ()).normalized();
    ...
    propertyThrustBodyHeadingAlignedToThrustInertialHeading(
        rHatUnit_SB_N, thrustHatUnit_B, sadaHatUnit_B, thrustReqHat_N.stableNormalized(), signOfN3Hat_N);
}
...
void fuzzPropertySolarArraySunOffsetBoundedByBodyThrustOffset(const Eigen::Vector3f& rHat_SB_N,
    ...
    propertySolarArraySunOffsetBoundedByBodyThrustOffset(
        rHatUnit_SB_N, thrustHatUnit_B, sadaHatUnit_B, thrustReqHatUnit_N, signOfN3Hat_N);
}
