...
Eigen::Vector3f referenceUpdate(const Eigen::Matrix3f& spacecraftInertia,
                                const InputGuidanceData& attGuidIn,
                                const RateControlAlgorithm& alg) {
    ...
    const Eigen::Vector3f Lr = -P * omega_BR_B + omega_RN_B.cross(ISCPntB_B * omega_BN_B) +
                               ISCPntB_B * (domega_RN_B - omega_BN_B.cross(omega_RN_B)) - knownTorquePntB_B;  // [Nm]
    return Lr;
}
...
static Eigen::Matrix3f makeSPDInertia(const std::array<float, 6>& p) {
    ...
    Eigen::Matrix3f M;
    M << p[0], p[1], p[2], p[1], p[3], p[4], p[2], p[4], p[5];
...
inline void fuzzAdapter_rateControl(const std::array<float, 6>& inertiaParams,
    ...
    if (!spacecraftInertia.allFinite() || !knownTorquePntB_B.allFinite() || !omega_BR_B.allFinite() ||
        !omega_RN_B.allFinite() || !domega_RN_B.allFinite()) {
        return;
    }
