...
static auto Vec3(float lo, float hi) {
    return fuzztest::ArrayOf<3>(fuzztest::InRange(lo, hi));
}

FUZZ_TEST(rateControlFuzz, fuzzAdapter_rateControl)
    .WithDomains(
        fuzztest::ArrayOf<6>(fuzztest::InRange(-100.0f, 100.0f)),  // inertia upper-tri params
