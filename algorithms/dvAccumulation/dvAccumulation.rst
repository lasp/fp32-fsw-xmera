...
subtracts the accelerometer bias :math:`\mathbf{b}` that the caller supplies. Then it integrates
the remainder during the configured control period :math:`\Delta t`. Thus each sample gives
...
An example is a small acceleration of 0.005 m/s^2 during a maximum burn of 1.5 hours. The
``controlPeriod`` is 0.2 s, thus there are 27,000 updates and the accumulator gets to 27 m/s. Each
``+=`` rounds to the nearest value that ``float`` can hold. Thus the relative limit on the
accumulated error is :math:`N \cdot 2^{-24}` for ``float`` and :math:`N \cdot 2^{-53}` for
``double``:
...
An example is an acceleration of 0.5 m/s^2. The ``controlPeriod`` is 0.2 s, thus the increment in
...
(here 1.7e6 m/s). After that point the accumulator stops. There is no trap and no infinity. The
module cannot get to these two limits in operation.
